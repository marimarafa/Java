...
public class ServiceImpiegatoImpl implements ServiceImpiegato {
	...
	public List<ImpiegatoNomeCognomeDTO> NomiCognomiImpiegati() {
		List<Impiegato> impiegati = dao.findAll();
		ArrayList<ImpiegatoNomeCognomeDTO> impiegatiDTO = new ArrayList<>();
		for(Impiegato imp : impiegati) {
			impiegatiDTO.add(new ImpiegatoNomeCognomeDTO(imp.getNome(), imp.getCognome()));
		}
		return impiegatiDTO;
	}
	...
	public List<ImpiegatoDTO> salarioMaggiore(int salario) {
		List<Impiegato> impiegati = dao.SalarioMaggiore(salario);
		ArrayList<ImpiegatoDTO> impiegatiDTO = new ArrayList<>();
		for(Impiegato imp :impiegati) {
			impiegatiDTO.add(Conversioni.daImpiegatoAImpiegatoDto(imp));
		}
	...
	public List<ImpiegatoDTO> impiegatiOrdinatiPerCognome() {
		List<Impiegato> impiegati = dao.impiegatiOrdinatiPerCognome();
		ArrayList<ImpiegatoDTO> impiegatiDTO = new ArrayList<>();
		for(Impiegato imp :impiegati) {
			impiegatiDTO.add(Conversioni.daImpiegatoAImpiegatoDto(imp));
		}
	...
	public List<ImpiegatoDTO> impiegatiOrdinatiPerSalario() {
		List<Impiegato> impiegati = dao.impiegatiOrdinatiPerSalario();
		ArrayList<ImpiegatoDTO> impiegatiDTO = new ArrayList<>();
		for(Impiegato imp :impiegati) {
			impiegatiDTO.add(Conversioni.daImpiegatoAImpiegatoDto(imp));
		}
